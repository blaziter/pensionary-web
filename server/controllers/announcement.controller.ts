...
export const createAnnouncement = (req: Request, res: Response) => {
  ...
  Firebird.attach(options, (err, db) => {
    ...
    db.query(
      'INSERT INTO ANNOUNCEMENTS (ANNOUNCEMENTID, TITLE, ANNOUNCEMENT) VALUES (?, ?, ?);',
      [
    ...
    db.query(
      ...
      (err, result) => {
        ...
        if (data.announcement)
          announcement[0].ANNOUNCEMENT = data?.announcement;

        db.query(
          'UPDATE ANNOUNCEMENTS set title = ?, announcement = ? WHERE announcementId = ?',
          [announcement[0].TITLE, announcement[0].ANNOUNCEMENT, announcementId],
          (err: string) => {
            if (err) {
              console.log(err);
              db.detach();
