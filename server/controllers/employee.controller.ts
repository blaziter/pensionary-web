...
export const newEmployee = (req: Request, res: Response) => {
    ...
    Firebird.attach(options, (err, db) => {
        ...
        db.query('INSERT INTO EMPLOYEE (EMPLOYEEID, SUFFIX, PREFIX, NAME, ROLE, AVAILABILITY, SHIFT, WORKPLACE, IMAGENAME) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);', [employee.employeeId, employee.suffix, employee.prefix, employee.name, employee.role, employee.availability, employee.shift, employee.workplace, employee.imagename], (err, result) => {
            if (err) {
                console.log(err);
                db.detach();
