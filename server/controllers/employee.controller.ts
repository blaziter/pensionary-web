...
export const newEmployee = (req: Request, res: Response) => {
  ...
  Firebird.attach(options, (err, db) => {
    ...
    db.query(
      'INSERT INTO EMPLOYEE (EMPLOYEEID, SUFFIX, PREFIX, NAME, ROLE, AVAILABILITY, SHIFT, WORKPLACE, IMAGENAME) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);',
      [
        employee.employeeId,
        employee.suffix,
        employee.prefix,
  ...
  Firebird.attach(options, (err, db) => {
    ...
    db.query(
      ...
      (err, result) => {
        ...
        db.query(
          'UPDATE EMPLOYEE set availability = ?, name = ?, prefix = ?, suffix = ?, role = ?, shift = ?, workplace = ?, WHERE employeeId = ?',
          [
            user[0].AVAILABILITY,
            user[0].NAME,
            user[0].PREFIX,
...
export const getEmployeeByParams = (req: Request, res: Response) => {
  ...
  Firebird.attach(options, (err, db) => {
    ...
    db.query(
      `SELECT * FROM employee WHERE ${group} = ?`,
      [data],
      (err, result) => {
        if (err) {
          console.log(err);
